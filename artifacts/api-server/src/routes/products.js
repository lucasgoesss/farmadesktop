...
router.get("/products", async (req, res) => {
    try {
        const { q, category, controlled } = req.query;
        ...
        if (controlled !== undefined)
            rows = rows.filter((p) => p.isControlled === (controlled === "true"));
...
router.patch("/products/:id", async (req, res) => {
    try {
        ...
        if (body.genericName !== undefined)
            update.genericName = body.genericName;
        if (body.barcode !== undefined)
            update.barcode = body.barcode;
        if (body.category !== undefined)
            update.category = body.category;
        if (body.manufacturer !== undefined)
            update.manufacturer = body.manufacturer;
        if (body.dosage !== undefined)
            update.dosage = body.dosage;
        if (body.unit !== undefined)
            update.unit = body.unit;
        if (body.costPrice !== undefined)
            update.costPrice = String(body.costPrice);
        if (body.salePrice !== undefined)
            update.salePrice = String(body.salePrice);
        if (body.minStock !== undefined)
            update.minStock = body.minStock;
        if (body.isControlled !== undefined)
            update.isControlled = body.isControlled;
        if (body.anvisaCode !== undefined)
            update.anvisaCode = body.anvisaCode;
        if (body.requiresPrescription !== undefined)
            update.requiresPrescription = body.requiresPrescription;
