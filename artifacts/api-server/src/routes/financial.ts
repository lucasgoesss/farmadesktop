...
const toTx = (t: typeof financialTransactionsTable.$inferSelect) => ({
  ...t,
...
router.get("/financial/cashflow", async (req, res): Promise<void> => {
  try {
    const [receivable] = await db
      .select({ total: sql<number>`coalesce(sum(${financialTransactionsTable.amount}),0)` })
...
router.get("/financial/accounts-payable", async (req, res): Promise<void> => {
  try {
    const rows = await db
      .select()
      .from(financialTransactionsTable)
...
router.get("/financial/accounts-receivable", async (req, res): Promise<void> => {
  try {
    const rows = await db
      .select()
      .from(financialTransactionsTable)
